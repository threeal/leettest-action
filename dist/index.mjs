...
function generateCppTest(schema, solutionFile, outFile) {
    ...
    mkdirSync(path$1.dirname(outFile), { recursive: true });
    writeFileSync(outFile, [
        `#include "${path$1.relative(path$1.dirname(outFile), solutionFile)}"`,
        ``,
        [...main.headers]
            .sort()
...
function createTestCppSolutionTasks(solutionFile) {
    const schemaFile = path$1.join(path$1.dirname(solutionFile), "test.yaml");
    const testFile = path$1.join("build", path$1.dirname(solutionFile), "test.cpp");
    const testExec = path$1.join(path$1.dirname(testFile), "test");
    return [
        {
...
var DefaultRenderer = class _DefaultRenderer {
  ...
  async render() {
    const { createLogUpdate } = await import('./index-BUx9ud_I.js');
